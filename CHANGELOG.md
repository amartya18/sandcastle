...
- 83a86f6: Add no-sandbox provider for interactive mode. `noSandbox()` runs the agent directly on the host with no container isolation — only accepted by `interactive()`, not `run()` or `createSandbox()`. The agent does not receive `--dangerously-skip-permissions`, so the user manages permissions themselves. Import from `@amartya18/sandcastle/sandboxes/no-sandbox`.
...
- 16315da: Add Daytona isolated sandbox provider (`@amartya18/sandcastle/sandboxes/daytona`)
...
- 5b04e73: ### Breaking changes
  ...
  - `docker()` factory is exported exclusively from `@amartya18/sandcastle/sandboxes/docker`
